    ...
    <div className="max-w-md mx-auto mt-10 bg-white rounded-xl shadow p-8">
      ...
      {step === 1 && (
        <form onSubmit={handleSendOtp} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white font-medium py-2.5 rounded-lg transition cursor-pointer">
      ...
      {step === 2 && (
        <form onSubmit={handleVerifyOtp} className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded p-2.5 text-xs text-amber-800">
            ℹ️ In demo mode, use OTP <span className="font-bold">123456</span> or check server log.
          </div>
          <div>
            ...
            <input
              ...
              placeholder="123456"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 tracking-widest text-center focus:ring-2 focus:ring-brand-500 focus:outline-none font-mono text-lg"
            />
          </div>
          {error && <p className="text-sm text-red-600">{error}</p>}
          <button type="submit" disabled={loading} className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white font-medium py-2.5 rounded-lg transition cursor-pointer">
