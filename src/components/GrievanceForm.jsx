    ...
    try {
      ...
      if (res.data) {
        ...
        if (!res.data.is_valid_civic_issue) {
          ...
          showToast(`✨ Identified: ${res.data.detected_issue_summary || res.data.suggested_category}`)
        }
        if (res.data.generated_complaint_text) {
                    ...
                    <div className="mt-3 space-y-2">
                      <div className="flex items-center justify-center gap-3">
                        <img
                          ...
                          className="h-28 w-36 object-cover rounded-lg border border-gray-200 shadow-sm"
                        />
                          ...
                          {aiAnalysis && (
                            <div className="space-y-1">
                              <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800">
                                <span>{CATEGORY_ICONS[aiAnalysis.suggested_category] || '🔍'}</span>
                                <span className="capitalize">{aiAnalysis.suggested_category?.replace('_', ' ')}</span>
                                <span>(Urgency: {aiAnalysis.urgency_score}/10)</span>
                              </span>
                              <p className="text-xs text-gray-600 font-medium">
                                {aiAnalysis.detected_issue_summary}
                              </p>
                            </div>
                          )}
                          <button
                            ...
                            className="text-xs text-rose-600 hover:underline block pt-1"
                          >
                            Remove Photo
                          </button>
                        </div>
